...
- Codex custom prompts are user-home scoped and deprecated; do not create repo-local `/prompts:*` as the durable path.
...
When command parity is requested, inspect `commands/README.md`, `commands/registry.json`, `plugin_hub/registry.json`, and `claude-plugins/` before making claims.
