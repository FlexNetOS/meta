...
This workspace is a meta-repo: independent child git repositories managed by `meta`, not a monorepo.

Workflow:

1. Run `meta project list --json` at session start or before cross-repo assumptions.
2. Run `meta git status` before editing, committing, summarizing, or stopping.
3. Prefer `meta exec`, `meta git`, `meta project`, `meta rust`, and `meta plugin` over ad hoc per-repo loops.
...
- Worker runs must create an isolated meta worktree with `meta git worktree create <name> --repo . --repo <repo> --json`.
