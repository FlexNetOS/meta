...
| MCP | `meta-mcp` and `git-kb mcp` expose tools to assistants. | Client registration and ownership policy are configured. | Single-owner MCP registration is an active policy task. | Duplicate or stale registrations must be audited manually. |
