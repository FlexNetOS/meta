...
1. **Command syntax**: Use `git-kb` (hyphen), not the unavailable `git kb` subcommand
...
**Prerequisite:** Code must be indexed first with `git-kb code index`. For semantic search, embeddings must be generated with `git-kb ai semantic`.
