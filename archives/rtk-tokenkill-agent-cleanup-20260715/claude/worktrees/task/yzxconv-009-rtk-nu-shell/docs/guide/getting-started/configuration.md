...
[shell]
mode = "legacy"             # "legacy" (default) or "nu_shell" — see Shell integration mode
```
...
`[shell] mode` selects which shell integration owns RTK command routing:
...
| Mode | Behavior |
|------|----------|
| `legacy` (default) | `rtk init` emits rewrite hooks into agent settings: the Claude Code settings.json PreToolUse entry, the Cursor hooks.json entry, and the Gemini bash hook script. |
| `nu_shell` | Native Nushell integration. Commands route through the rtk binary via Nushell wrapper defs (e.g. `export def --wrapped git [...rest] { ^rtk git ...$rest }`), so `rtk init` does **not** emit those rewrite hooks. Instruction files (RTK.md, CLAUDE.md/AGENTS.md references) are still installed, and `rtk init` still removes legacy `rtk-rewrite.sh` scripts it finds. |

`rtk config` prints the current selection in its `[shell]` section. Unknown mode values are rejected at config parse time.

The `nu_shell` mode does not install Nushell wrappers itself — the Nushell environment (for example Yazelix's `rtk_wrappers.nu`) owns those definitions. Selecting `nu_shell` declares that ownership and keeps `rtk init` from layering bash/zsh rewrite hooks on top of it. Plugin-based integrations that are not shell hooks (OpenCode/Pi TypeScript plugins, Hermes Python plugin, Copilot JSON, rules files) are unaffected.
