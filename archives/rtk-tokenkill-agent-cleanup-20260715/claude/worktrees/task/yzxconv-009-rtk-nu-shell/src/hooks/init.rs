...
/// True when config.toml selects native Nushell integration
/// (`[shell] mode = "nu_shell"`): commands route through the rtk binary via
/// Nushell wrapper defs, so `rtk init` must not emit legacy bash/zsh rewrite
/// hooks (Claude settings.json PreToolUse entries, Cursor hooks.json entries,
/// the Gemini bash hook script).
fn nu_shell_mode_selected() -> bool {
...
fn run_default_mode(
    ...
    let nu_shell = nu_shell_mode_selected();

    // 4. Print success message (skip in dry-run)
    if !dry_run {
        if nu_shell {
            println!("\nRTK registered (global).\n");
            println!("  Shell mode: nu_shell (native Nushell integration)");
        } else {
    ...
    // 5. Patch settings.json with binary command — unless nu_shell mode is
    //    selected: native Nushell integration needs no bash rewrite hook.
    let patch_result = if nu_shell {
        if dry_run {
            println!("[dry-run] would skip settings.json hook ([shell] mode = \"nu_shell\")");
        } else {
            println!(
                "\n  settings.json: skipped — [shell] mode = \"nu_shell\" routes commands through Nushell wrappers; no bash rewrite hook installed"
            );
        }
        PatchResult::Skipped
    } else {
...
fn run_hook_only_mode(
    ...
    if nu_shell_mode_selected() {
        println!("\nRTK hook not registered: [shell] mode = \"nu_shell\" selects native Nushell integration.");
        println!("  Commands route through the rtk binary via Nushell wrappers; no bash rewrite hook installed.");
        return Ok(());
    }
...
fn install_cursor_hooks(ctx: InitContext) -> Result<()> {
    let InitContext { verbose, dry_run } = ctx;
    if nu_shell_mode_selected() {
        println!("\nCursor hook skipped: [shell] mode = \"nu_shell\" selects native Nushell integration (no rewrite hook).");
        return Ok(());
    }
...
pub fn run_gemini(
    ...
    if nu_shell_mode_selected() {
        println!("\nGemini hook skipped: [shell] mode = \"nu_shell\" selects native Nushell integration (no bash rewrite hook).");
        return Ok(());
    }
