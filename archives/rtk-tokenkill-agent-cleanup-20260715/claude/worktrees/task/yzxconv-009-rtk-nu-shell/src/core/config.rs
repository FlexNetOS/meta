...
/// Which shell integration owns RTK command routing.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Default)]
#[serde(rename_all = "snake_case")]
pub enum ShellMode {
    /// Legacy bash/zsh hook integration (default): `rtk init` emits
    /// PreToolUse rewrite hooks into agent settings (Claude settings.json,
    /// Cursor hooks.json, Gemini bash hook script).
    #[default]
    Legacy,
    /// Native Nushell integration: commands route through the rtk binary
    /// via Nushell wrapper defs (e.g. Yazelix `rtk_wrappers.nu`), so
    /// `rtk init` must NOT emit legacy bash/zsh rewrite hooks.
    NuShell,
}

#[derive(Debug, Serialize, Deserialize, Default)]
pub struct ShellConfig {
    /// `mode = "nu_shell"` selects native Nushell integration;
    /// `mode = "legacy"` (default) keeps the bash/zsh rewrite hook flow.
    #[serde(default)]
    pub mode: ShellMode,
}

impl ShellConfig {
    pub fn is_nu_shell(&self) -> bool {
        self.mode == ShellMode::NuShell
    }
}
...
"#;
    ...
    #[test]
    fn test_shell_mode_nu_shell_deserialize() {
        let toml = r#"
[shell]
mode = "nu_shell"
"#;
        let config: Config = toml::from_str(toml).expect("valid toml");
        assert_eq!(config.shell.mode, ShellMode::NuShell);
        assert!(config.shell.is_nu_shell());
    }

    #[test]
    fn test_shell_mode_default_is_legacy() {
        // Configs that predate the [shell] section keep the legacy hook flow.
        let toml = r#"
[tracking]
enabled = true
history_days = 90
"#;
        let config: Config = toml::from_str(toml).expect("valid toml");
        assert_eq!(config.shell.mode, ShellMode::Legacy);
        assert!(!config.shell.is_nu_shell());
        assert!(!Config::default().shell.is_nu_shell());
    }

    #[test]
    fn test_shell_mode_unknown_value_is_rejected() {
        let toml = r#"
[shell]
mode = "fish"
"#;
        assert!(
            ...
            "only documented shell modes (legacy, nu_shell) parse"
        );
    }

    #[test]
    fn test_shell_mode_serializes_for_rtk_config_output() {
        // `rtk config` prints toml::to_string_pretty(&config): the selection
        // must appear as a documented [shell] section.
        let mut config = Config::default();
        config.shell.mode = ShellMode::NuShell;
        let out = toml::to_string_pretty(&config).expect("serialize");
        assert!(out.contains("[shell]"), "missing [shell] section: {out}");
        assert!(
            out.contains("mode = \"nu_shell\""),
            "missing selection: {out}"
        );

        let default_out = toml::to_string_pretty(&Config::default()).expect("serialize");
        assert!(
            default_out.contains("mode = \"legacy\""),
            "default output must document the field: {default_out}"
        );
    }
}
