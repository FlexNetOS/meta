...
/// Runtime integrity gate retained for compatibility with the command surface.
///
/// Claude now runs `rtk hook claude` directly from `settings.json`; it has no
/// installed script to hash. A leftover legacy script must never prevent an
/// operational RTK command from running, so this gate is deliberately a no-op.
/// `rtk init -g` removes that obsolete script and sidecar during migration.
