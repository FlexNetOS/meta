...
# FlexNetOS CLI Compatibility

In this workspace, use `/home/flexnetos/FlexNetOS/usr/bin/git-kb` or `git-kb` if that frontdoor is on `PATH`. The current installed `git-kb 0.2.12` checkout command accepts pathspecs such as `git-kb checkout context/`; do not use the stale `git-kb checkout context/` form.

Bootstrap semantics:

- `git-kb init` creates/configures a KB.
- `git-kb init codex` installs Codex integration files.
- `meta init claude` installs Meta-owned Claude integration files in current `meta 0.2.22`.
- `meta exec -- <cmd>` runs `<cmd>` across repos; there is no `meta exec init` subcommand.
...
|-------------------|---------------------------------------------|
| Bug report | `git-kb create --type incident --title "Brief description" && git-kb commit -m "Report bug"` |
| Feature request | `git-kb create --type task --title "Brief description" && git-kb commit -m "Feature request"` |
| "Fix this" / "There's a problem" | `git-kb create --type incident --title "Brief description" && git-kb commit -m "Report incident"` |
